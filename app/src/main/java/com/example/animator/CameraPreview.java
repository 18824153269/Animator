...
//    private Thread mThread;
    ...
    private static IVolumeListener mListener;
    public boolean isGetVolume;

    public static void setVolumeListener(IVolumeListener listener) {
        mListener = listener;
    }
...
//        ExecutorService thread = Executors.newSingleThreadExecutor();
                ...
                while (isGetVolume) {
                    if (mMediarecorder != null) {
                        try {
                            ...
                            mListener.onResult(Calculator.dbstart);
    ...
    private void initTex() {
        ...
        GLES20.glGenTextures(2, mTextureId, 0);  //生成纹理
        GLES20.glBindTexture(GLES11Ext.GL_TEXTURE_EXTERNAL_OES, mTextureId[0]); //生成纹理
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        GLES20.glUseProgram(hProgram); // 将program加入OpenGL ES环境中

        int ph = GLES20.glGetAttribLocation(hProgram, "vPosition");  // 获取指向vertex shader的成员vPosition的 handle
        int tch = GLES20.glGetAttribLocation(hProgram, "vTexCoord");
        int th = GLES20.glGetUniformLocation(hProgram, "sTexture"); // 访问顶点着色器中定义的矩阵变量
        ...
        GLES20.glEnableVertexAttribArray(ph); // 启用一个指向三角形的顶点数组的handle
...

        {
            ...
            GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP, 0, 4); // 画三角形
...
//        GLES20.glViewport(0, 0, mWindowWidth, mWindowHeight);
        ...
        GLES20.glDisableVertexAttribArray(ph); // 禁用指向三角形的顶点数组
    ...
    private static int loadShader(String vss, String fss) {
        int vshader = GLES20.glCreateShader(GLES20.GL_VERTEX_SHADER); //创建一个顶点类型的shader
        GLES20.glShaderSource(vshader, vss); // 将源码添加到shader并编译之
        GLES20.glCompileShader(vshader); // 将源码添加到shader并编译之
        ...
        GLES20.glAttachShader(program, vshader); // 将vertex shader添加到program
        GLES20.glAttachShader(program, fshader); // 将fragment shader添加到program
